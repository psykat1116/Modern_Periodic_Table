...
const CopperAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const copperNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(copperNucleus);

    // Create electrons for copper (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 1 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the copper nucleus
      copperNucleus.rotation.x += 0.01;
      copperNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 29; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 1));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 1));
      }
