...
const AluminumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const aluminumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(aluminumNucleus);

    // Create electrons for aluminum (2 in the first orbit, 8 in the second orbit, 3 in the third orbit)
    ...
    const animate = function () {
      ...
      // Rotate the aluminum nucleus
      aluminumNucleus.rotation.x += 0.01;
      aluminumNucleus.rotation.y += 0.01;
