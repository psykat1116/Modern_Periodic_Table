...
const TelluriumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Tellurium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const telluriumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(telluriumNucleus);

    // Create electrons for tellurium
    ...
    const animate = function () {
      ...
      // Rotate the tellurium nucleus
      telluriumNucleus.rotation.x += 0.01;
      telluriumNucleus.rotation.y += 0.01;
      ...
      for (let i = 46; i < 52; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 46) * (2 * Math.PI / 6));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 46) * (2 * Math.PI / 6));
      }
