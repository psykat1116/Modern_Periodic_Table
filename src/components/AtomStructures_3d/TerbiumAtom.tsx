...
const TerbiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const terbiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(terbiumNucleus);

    // Create electrons for terbium
    ...
    const orbitRadius6 = 18; // Adding a new orbit for Terbium
    ...
    const orbitGeometry6 = new THREE.CircleGeometry(orbitRadius6, orbitSegments); // New orbit for Terbium
    ...
    const electronOrbit6 = new THREE.LineSegments(new THREE.EdgesGeometry(orbitGeometry6), orbitMaterial); // New orbit for Terbium
    ...
    const animate = function () {
      ...
      // Rotate the terbium nucleus
      terbiumNucleus.rotation.x += 0.01;
      terbiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 55; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 27));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 27));
      }

      for (let i = 55; i < 63; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 55) * (2 * Math.PI / 8));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 55) * (2 * Math.PI / 8));
      }

      for (let i = 63; i < 65; i++) {
        electrons[i].position.x = orbitRadius6 * Math.cos(angle6 + (i - 63) * (2 * Math.PI / 2));
        electrons[i].position.y = orbitRadius6 * Math.sin(angle6 + (i - 63) * (2 * Math.PI / 2));
      }
