...
const IodineAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const iodineNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(iodineNucleus);

    // Create electrons for iodine
    ...
    // Create electron orbits for iodine
    ...
    const animate = function () {
      ...
      // Rotate the iodine nucleus
      iodineNucleus.rotation.x += 0.01;
      iodineNucleus.rotation.y += 0.01;
      ...
      for (let i = 46; i < 53; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 46) * (2 * Math.PI / 7));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 46) * (2 * Math.PI / 7));
      }
