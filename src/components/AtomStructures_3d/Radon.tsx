...
const RadonAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const radonNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(radonNucleus);

    // Create electrons for Radon
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Radon
    const electronCounts = [2, 8, 18, 32, 18, 8];
    ...
    const animate = function () {
      ...
      // Rotate the radon nucleus
      radonNucleus.rotation.x += 0.01;
      radonNucleus.rotation.y += 0.01;
