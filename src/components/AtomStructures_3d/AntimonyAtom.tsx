...
const AntimonyAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const antimonyNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(antimonyNucleus);

    // Create electrons for antimony (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 18 in the fourth orbit, 5 in the fifth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the antimony nucleus
      antimonyNucleus.rotation.x += 0.01;
      antimonyNucleus.rotation.y += 0.01;
      ...
      for (let i = 46; i < 51; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 46) * (2 * Math.PI / 5));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 46) * (2 * Math.PI / 5));
      }
