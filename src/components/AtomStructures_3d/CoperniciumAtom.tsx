...
const CoperniciumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Copernicium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const coperniciumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(coperniciumNucleus);

    // Create electrons for copernicium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Copernicium
    const electronCounts = [2, 8, 18, 32, 32, 18, 2];
    ...
    const animate = function () {
      ...
      // Rotate the copernicium nucleus
      coperniciumNucleus.rotation.x += 0.01;
      coperniciumNucleus.rotation.y += 0.01;
...
export default CoperniciumAtom;