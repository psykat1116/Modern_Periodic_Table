...
const SiliconAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const siliconNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(siliconNucleus);

    // Create electrons for silicon (2 in the first orbit, 8 in the second orbit, 4 in the third orbit)
    ...
    const animate = function () {
      ...
      // Rotate the silicon nucleus
      siliconNucleus.rotation.x += 0.01;
      siliconNucleus.rotation.y += 0.01;
