...
const MeitneriumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Meitnerium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const meitneriumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(meitneriumNucleus);

    // Create electrons for Meitnerium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Meitnerium
    const electronCounts = [2, 8, 18, 32, 32, 15, 2];
    ...
    const animate = function () {
      ...
      // Rotate the meitnerium nucleus
      meitneriumNucleus.rotation.x += 0.01;
      meitneriumNucleus.rotation.y += 0.01;
