...
const RutherfordiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Rutherfordium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const rutherfordiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(rutherfordiumNucleus);

    // Create electrons for Rutherfordium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Rutherfordium
    const electronCounts = [2, 8, 18, 32, 32, 10, 2];
    const orbitRadii = [3, 6, 9, 12, 15, 18, 21]; // Adjusted radii for Rutherfordium
    ...
    const animate = function () {
      ...
      // Rotate the rutherfordium nucleus
      rutherfordiumNucleus.rotation.x += 0.01;
      rutherfordiumNucleus.rotation.y += 0.01;
...
export default RutherfordiumAtom;