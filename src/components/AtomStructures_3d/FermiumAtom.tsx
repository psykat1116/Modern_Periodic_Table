...
const FermiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const fermiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(fermiumNucleus);

    // Create electrons for fermium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Fermium
    const electronCounts = [2, 8, 18, 32, 30, 8, 2];
    const orbitRadii = [3, 6, 9, 12, 15, 18, 21]; // Adjusted for Fermium
    ...
    const animate = function () {
      ...
      // Rotate the fermium nucleus
      fermiumNucleus.rotation.x += 0.01;
      fermiumNucleus.rotation.y += 0.01;
