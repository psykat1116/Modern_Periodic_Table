...
const NeodymiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Neodymium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const neodymiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(neodymiumNucleus);

    // Create electrons for neodymium
    ...
    const animate = function () {
      ...
      // Rotate the neodymium nucleus
      neodymiumNucleus.rotation.x += 0.01;
      neodymiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 50; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 22));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 22));
      }

      for (let i = 50; i < 58; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 50) * (2 * Math.PI / 8));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 50) * (2 * Math.PI / 8));
      }

      for (let i = 58; i < 60; i++) {
        electrons[i].position.x = orbitRadius6 * Math.cos(angle6 + (i - 58) * (2 * Math.PI / 2));
        electrons[i].position.y = orbitRadius6 * Math.sin(angle6 + (i - 58) * (2 * Math.PI / 2));
      }
