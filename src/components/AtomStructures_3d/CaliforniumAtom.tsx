...
const CaliforniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Californium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const californiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(californiumNucleus);

    // Create electrons for californium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Californium
    const electronCounts = [2, 8, 18, 32, 28, 8, 2];
    ...
    const animate = function () {
      ...
      // Rotate the californium nucleus
      californiumNucleus.rotation.x += 0.01;
      californiumNucleus.rotation.y += 0.01;
...
export default CaliforniumAtom;