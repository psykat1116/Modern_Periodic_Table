...
const PoloniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const poloniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(poloniumNucleus);

    // Create electrons for polonium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Polonium
    const electronCounts = [2, 8, 18, 32, 18, 6]; // Updated electron counts
    ...
    const animate = function () {
      ...
      // Rotate the polonium nucleus
      poloniumNucleus.rotation.x += 0.01;
      poloniumNucleus.rotation.y += 0.01;
