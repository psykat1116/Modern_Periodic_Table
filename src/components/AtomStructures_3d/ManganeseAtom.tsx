...
const ManganeseAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const manganeseNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(manganeseNucleus);

    // Create electrons for manganese (2 in the first orbit, 8 in the second orbit, 13 in the third orbit, 2 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the manganese nucleus
      manganeseNucleus.rotation.x += 0.01;
      manganeseNucleus.rotation.y += 0.01;

      // Move the electrons in orbits
      for (let i = 0; i < 2; i++) {
        electrons[i].position.x = orbitRadius1 * Math.cos(angle1 + i * (Math.PI));
        electrons[i].position.y = orbitRadius1 * Math.sin(angle1 + i * (Math.PI));
      }

      for (let i = 2; i < 10; i++) {
        electrons[i].position.x = orbitRadius2 * Math.cos(angle2 + (i - 2) * (Math.PI / 4));
        electrons[i].position.y = orbitRadius2 * Math.sin(angle2 + (i - 2) * (Math.PI / 4));
      }

      for (let i = 10; i < 23; i++) {
        electrons[i].position.x = orbitRadius3 * Math.cos(angle3 + (i - 10) * (Math.PI / 6));
        electrons[i].position.y = orbitRadius3 * Math.sin(angle3 + (i - 10) * (Math.PI / 6));
      }

      for (let i = 23; i < 25; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 23) * (Math.PI));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 23) * (Math.PI));
      }
