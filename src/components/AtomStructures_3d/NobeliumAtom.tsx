...
const NobeliumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const nobeliumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(nobeliumNucleus);

    // Create electrons for nobelium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Nobelium
    const electronCounts = [2, 8, 18, 32, 32, 8, 2];
    const orbitRadii = [3, 6, 9, 12, 15, 18, 21]; // Adjusted radii for Nobelium
    ...
    const animate = function () {
      ...
      // Rotate the nobelium nucleus
      nobeliumNucleus.rotation.x += 0.01;
      nobeliumNucleus.rotation.y += 0.01;
