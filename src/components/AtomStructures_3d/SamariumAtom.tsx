...
const SamariumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const samariumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(samariumNucleus);

    // Create electrons for samarium
    ...
    const animate = function () {
      ...
      // Rotate the samarium nucleus
      samariumNucleus.rotation.x += 0.01;
      samariumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 52; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 24));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 24));
      }

      for (let i = 52; i < 60; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 52) * (2 * Math.PI / 8));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 52) * (2 * Math.PI / 8));
      }

      for (let i = 60; i < 62; i++) {
        electrons[i].position.x = orbitRadius6 * Math.cos(angle6 + (i - 60) * (2 * Math.PI / 2));
        electrons[i].position.y = orbitRadius6 * Math.sin(angle6 + (i - 60) * (2 * Math.PI / 2));
      }
