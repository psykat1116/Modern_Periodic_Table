...
const BariumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const bariumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(bariumNucleus);

    // Create electrons for barium
    ...
    // Create electron orbits for barium
    ...
    const animate = function () {
      ...
      // Rotate the barium nucleus
      bariumNucleus.rotation.x += 0.01;
      bariumNucleus.rotation.y += 0.01;
      ...
      for (let i = 54; i < 56; i++) {
        electrons[i].position.x = orbitRadius6 * Math.cos(angle6 + (i - 54) * (2 * Math.PI / 2));
        electrons[i].position.y = orbitRadius6 * Math.sin(angle6 + (i - 54) * (2 * Math.PI / 2));
      }
