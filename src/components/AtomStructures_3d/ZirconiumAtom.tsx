...
const ZirconiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const zirconiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(zirconiumNucleus);

    // Create electrons for zirconium (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 10 in the fourth orbit, 2 in the fifth orbit)
    ...
    const orbitRadius5 = 15; // Add a new orbit for Zirconium
    ...
    const animate = function () {
      ...
      // Rotate the zirconium nucleus
      zirconiumNucleus.rotation.x += 0.01;
      zirconiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 38; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 10));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 10));
      }

      for (let i = 38; i < 40; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 38) * (2 * Math.PI / 2));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 38) * (2 * Math.PI / 2));
      }
