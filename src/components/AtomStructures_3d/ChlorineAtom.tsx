...
const ChlorineAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const chlorineNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(chlorineNucleus);

    // Create electrons for chlorine (2 in the first orbit, 8 in the second orbit, 7 in the third orbit)
    ...
    const animate = function () {
      ...
      // Rotate the chlorine nucleus
      chlorineNucleus.rotation.x += 0.01;
      chlorineNucleus.rotation.y += 0.01;
