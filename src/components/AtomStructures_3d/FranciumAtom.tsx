...
const FranciumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const franciumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(franciumNucleus);

    // Create electrons for Francium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Francium
    const electronCounts = [2, 8, 18, 32, 18, 8, 1];
    ...
    const animate = function () {
      ...
      // Rotate the francium nucleus
      franciumNucleus.rotation.x += 0.01;
      franciumNucleus.rotation.y += 0.01;
