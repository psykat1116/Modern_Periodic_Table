...
const NiobiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const niobiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(niobiumNucleus);

    // Create electrons for niobium (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 12 in the fourth orbit, 1 in the fifth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the niobium nucleus
      niobiumNucleus.rotation.x += 0.01;
      niobiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 40; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 12));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 12));
      }

      electrons[40].position.x = orbitRadius5 * Math.cos(angle5);
      electrons[40].position.y = orbitRadius5 * Math.sin(angle5);
