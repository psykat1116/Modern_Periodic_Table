...
const PhosphorusAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const phosphorusNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(phosphorusNucleus);

    // Create electrons for phosphorus (2 in the first orbit, 8 in the second orbit, 5 in the third orbit)
    ...
    const animate = function () {
      ...
      // Rotate the phosphorus nucleus
      phosphorusNucleus.rotation.x += 0.01;
      phosphorusNucleus.rotation.y += 0.01;
      ...
      for (let i = 10; i < 15; i++) {
        electrons[i].position.x = orbitRadius3 * Math.cos(angle3 + ((i - 10) * (2 * Math.PI / 5)));
        electrons[i].position.y = orbitRadius3 * Math.sin(angle3 + ((i - 10) * (2 * Math.PI / 5)));
      }
