...
const LanthanumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Lanthanum atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const lanthanumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(lanthanumNucleus);

    // Create electrons for lanthanum
    ...
    const animate = function () {
      ...
      // Rotate the lanthanum nucleus
      lanthanumNucleus.rotation.x += 0.01;
      lanthanumNucleus.rotation.y += 0.01;
      ...
      for (let i = 46; i < 55; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 46) * (2 * Math.PI / 9));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 46) * (2 * Math.PI / 9));
      }

      for (let i = 55; i < 57; i++) {
        electrons[i].position.x = orbitRadius6 * Math.cos(angle6 + (i - 55) * (2 * Math.PI / 2));
        electrons[i].position.y = orbitRadius6 * Math.sin(angle6 + (i - 55) * (2 * Math.PI / 2));
      }
