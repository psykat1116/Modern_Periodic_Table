...
const ChromiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const chromiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(chromiumNucleus);

    // Create electrons for chromium (2 in the first orbit, 8 in the second orbit, 13 in the third orbit, 1 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the chromium nucleus
      chromiumNucleus.rotation.x += 0.01;
      chromiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 10; i < 23; i++) {
        electrons[i].position.x = orbitRadius3 * Math.cos(angle3 + (i - 10) * (Math.PI / 12));
        electrons[i].position.y = orbitRadius3 * Math.sin(angle3 + (i - 10) * (Math.PI / 12));
      }

      electrons[23].position.x = orbitRadius4 * Math.cos(angle4);
      electrons[23].position.y = orbitRadius4 * Math.sin(angle4);
