...
const TennessineAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Tennessine atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const tennessineNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(tennessineNucleus);

    // Create electrons for tennessine
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Tennessine
    const electronCounts = [2, 8, 18, 32, 32, 18, 7];
    ...
    const animate = function () {
      ...
      // Rotate the tennessine nucleus
      tennessineNucleus.rotation.x += 0.01;
      tennessineNucleus.rotation.y += 0.01;
