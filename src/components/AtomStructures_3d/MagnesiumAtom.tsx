...
const MagnesiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Magnesium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const magnesiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(magnesiumNucleus);

    // Create electrons for magnesium (2 in the first orbit, 8 in the second orbit, 2 in the third orbit)
    ...
    const animate = function () {
      ...
      // Rotate the magnesium nucleus
      magnesiumNucleus.rotation.x += 0.01;
      magnesiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 10; i < 12; i++) {
        electrons[i].position.x = orbitRadius3 * Math.cos(angle3 + (i - 10) * (Math.PI / 2));
        electrons[i].position.y = orbitRadius3 * Math.sin(angle3 + (i - 10) * (Math.PI / 2));
      }
