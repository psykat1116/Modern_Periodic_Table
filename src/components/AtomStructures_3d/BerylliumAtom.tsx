...
const BerylliumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const berylliumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(berylliumNucleus);

    // Create electrons for beryllium (2 electrons in the first orbit, 2 electrons in the second orbit)
    ...
    const animate = function () {
      ...
      // Rotate the beryllium nucleus
      berylliumNucleus.rotation.x += 0.01;
      berylliumNucleus.rotation.y += 0.01;
      ...
      electrons[2].position.x = orbitRadius2 * Math.cos(angle2);
      electrons[2].position.y = orbitRadius2 * Math.sin(angle2);

      electrons[3].position.x = orbitRadius2 * Math.cos(angle2 + Math.PI);
      electrons[3].position.y = orbitRadius2 * Math.sin(angle2 + Math.PI);
