...
const IridiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const iridiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(iridiumNucleus);

    // Create electrons for iridium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Iridium
    const electronCounts = [2, 8, 18, 32, 15, 2];
    ...
    const animate = function () {
      ...
      // Rotate the iridium nucleus
      iridiumNucleus.rotation.x += 0.01;
      iridiumNucleus.rotation.y += 0.01;
