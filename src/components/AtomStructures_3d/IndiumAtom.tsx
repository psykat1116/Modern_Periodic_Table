...
const IndiumAtom = ({ theme, containerSize }) => {
  ...
  useEffect(() => {
    ...
    const indiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(indiumNucleus);

    // Create electrons for indium
    const electrons = [];
    ...
    const orbitRadius5 = 15; // New orbit for Indium
    const orbitSegments = 64;

    // Add electrons to the scene
    for (let i = 0; i < 2; i++) {
      electrons.push(createElectron(electronMaterial, 2, i + 1, orbitRadius1));
    }

    for (let i = 0; i < 8; i++) {
      electrons.push(createElectron(electronMaterial, 4, i + 1, orbitRadius2));
    }

    for (let i = 0; i < 18; i++) {
      electrons.push(createElectron(electronMaterial, 6, i + 1, orbitRadius3));
    }

    for (let i = 0; i < 18; i++) {
      electrons.push(createElectron(electronMaterial, 8, i + 1, orbitRadius4));
    }

    for (let i = 0; i < 3; i++) {
      electrons.push(createElectron(electronMaterial, 10, i + 1, orbitRadius5));
    }
    ...
    const animate = function () {
      ...
      // Rotate the indium nucleus
      indiumNucleus.rotation.x += 0.01;
      indiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 46; i < 49; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 46) * (2 * Math.PI / 3));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 46) * (2 * Math.PI / 3));
      }
    ...
    function createElectron(material, radius, distance, orbitRadius) {
      const electronGeometry = new THREE.SphereGeometry(0.2, 16, 16);
      const electron = new THREE.Mesh(electronGeometry, material);

      electron.position.x = orbitRadius * Math.cos(distance * (2 * Math.PI / radius));
      electron.position.y = orbitRadius * Math.sin(distance * (2 * Math.PI / radius));
