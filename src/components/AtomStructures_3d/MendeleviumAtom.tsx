...
const MendeleviumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Mendelevium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const mendeleviumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(mendeleviumNucleus);

    // Create electrons for Mendelevium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Mendelevium
    const electronCounts = [2, 8, 18, 32, 31, 8, 2];
    ...
    const animate = function () {
      ...
      // Rotate the mendelevium nucleus
      mendeleviumNucleus.rotation.x += 0.01;
      mendeleviumNucleus.rotation.y += 0.01;
...
export default MendeleviumAtom;