...
const LawrenciumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Lawrencium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const lawrenciumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(lawrenciumNucleus);

    // Create electrons for lawrencium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Lawrencium
    const electronCounts = [2, 8, 18, 32, 32, 9, 2];
    ...
    const animate = function () {
      ...
      // Rotate the lawrencium nucleus
      lawrenciumNucleus.rotation.x += 0.01;
      lawrenciumNucleus.rotation.y += 0.01;
