...
const BromineAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const bromineNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(bromineNucleus);

    // Create electrons for bromine (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 7 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the bromine nucleus
      bromineNucleus.rotation.x += 0.01;
      bromineNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 35; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 7));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 7));
      }
