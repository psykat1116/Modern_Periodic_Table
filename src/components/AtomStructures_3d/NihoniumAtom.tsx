...
const NihoniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const nihoniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(nihoniumNucleus);

    // Create electrons for Nihonium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Nihonium
    const electronCounts = [2, 8, 18, 32, 32, 18, 3];
    ...
    const animate = function () {
      ...
      // Rotate the nihonium nucleus
      nihoniumNucleus.rotation.x += 0.01;
      nihoniumNucleus.rotation.y += 0.01;
