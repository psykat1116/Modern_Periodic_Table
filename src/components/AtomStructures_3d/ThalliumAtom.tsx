...
const ThalliumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const thalliumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(thalliumNucleus);

    // Create electrons for thallium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Thallium
    const electronCounts = [2, 8, 18, 32, 18, 3];
    ...
    const animate = function () {
      ...
      // Rotate the thallium nucleus
      thalliumNucleus.rotation.x += 0.01;
      thalliumNucleus.rotation.y += 0.01;
