...
const RoentgeniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Roentgenium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const roentgeniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(roentgeniumNucleus);

    // Create electrons for Roentgenium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Roentgenium
    const electronCounts = [2, 8, 18, 32, 32, 17, 2]; // Updated electron counts
    ...
    const animate = function () {
      ...
      // Rotate the roentgenium nucleus
      roentgeniumNucleus.rotation.x += 0.01;
      roentgeniumNucleus.rotation.y += 0.01;
...
export default RoentgeniumAtom;