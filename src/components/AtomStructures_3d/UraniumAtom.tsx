...
const UraniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const uraniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(uraniumNucleus);

    // Create electrons for uranium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Uranium
    const electronCounts = [2, 8, 18, 32, 21, 9, 2];
    ...
    const animate = function () {
      ...
      // Rotate the uranium nucleus
      uraniumNucleus.rotation.x += 0.01;
      uraniumNucleus.rotation.y += 0.01;
