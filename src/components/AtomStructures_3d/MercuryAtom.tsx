...
const MercuryAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const mercuryNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(mercuryNucleus);

    // Create electrons for Mercury
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Mercury
    const electronCounts = [2, 8, 18, 32, 18, 2]; // Electron counts for Mercury
    const orbitRadii = [3, 6, 9, 12, 15, 18]; // Orbit radii for Mercury
    ...
    const animate = function () {
      ...
      // Rotate the mercury nucleus
      mercuryNucleus.rotation.x += 0.01;
      mercuryNucleus.rotation.y += 0.01;
