...
const PlatinumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const platinumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(platinumNucleus);

    // Create electrons for platinum
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Platinum
    const electronCounts = [2, 8, 18, 32, 17, 1];
    ...
    const animate = function () {
      ...
      // Rotate the platinum nucleus
      platinumNucleus.rotation.x += 0.01;
      platinumNucleus.rotation.y += 0.01;
