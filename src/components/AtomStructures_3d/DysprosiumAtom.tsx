...
const DysprosiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Dysprosium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const dysprosiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(dysprosiumNucleus);

    // Create electrons for dysprosium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Dysprosium
    const electronCounts = [2, 8, 18, 28, 8, 2];
    ...
    const animate = function () {
      ...
      // Rotate the dysprosium nucleus
      dysprosiumNucleus.rotation.x += 0.01;
      dysprosiumNucleus.rotation.y += 0.01;
