...
const HolmiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const holmiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(holmiumNucleus);

    // Create electrons for holmium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Holmium
    const electronCounts = [2, 8, 18, 29, 8, 2];  // Updated electron counts for Holmium
    ...
    const animate = function () {
      ...
      // Rotate the holmium nucleus
      holmiumNucleus.rotation.x += 0.01;
      holmiumNucleus.rotation.y += 0.01;
