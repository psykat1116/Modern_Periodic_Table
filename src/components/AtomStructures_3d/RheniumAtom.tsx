...
const RheniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const rheniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(rheniumNucleus);

    // Create electrons for Rhenium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Rhenium
    const electronCounts = [2, 8, 18, 32, 13, 2]; // Updated electron counts for Rhenium
    const orbitRadii = [3, 6, 9, 12, 15, 18]; // Updated orbit radii
    ...
    const animate = function () {
      ...
      // Rotate the rhenium nucleus
      rheniumNucleus.rotation.x += 0.01;
      rheniumNucleus.rotation.y += 0.01;
