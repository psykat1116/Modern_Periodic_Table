...
const ThoriumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const thoriumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(thoriumNucleus);

    // Create electrons for thorium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Thorium
    const electronCounts = [2, 8, 18, 32, 18, 10, 2]; // Adjusted for Thorium
    const orbitRadii = [3, 6, 9, 12, 15, 18, 21]; // Adjusted for Thorium
    ...
    const animate = function () {
      ...
      // Rotate the thorium nucleus
      thoriumNucleus.rotation.x += 0.01;
      thoriumNucleus.rotation.y += 0.01;
