...
const ZincAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const zincNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(zincNucleus);

    // Create electrons for zinc (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 2 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the zinc nucleus
      zincNucleus.rotation.x += 0.01;
      zincNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 30; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 2));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 2));
      }
