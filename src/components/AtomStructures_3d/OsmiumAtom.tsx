...
const OsmiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const osmiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(osmiumNucleus);

    // Create electrons for osmium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Osmium
    const electronCounts = [2, 8, 18, 32, 14, 2]; // Updated electron counts
    ...
    const animate = function () {
      ...
      // Rotate the osmium nucleus
      osmiumNucleus.rotation.x += 0.01;
      osmiumNucleus.rotation.y += 0.01;
