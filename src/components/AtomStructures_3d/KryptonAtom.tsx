...
const KryptonAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const kryptonNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(kryptonNucleus);

    // Create electrons for krypton (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 8 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the krypton nucleus
      kryptonNucleus.rotation.x += 0.01;
      kryptonNucleus.rotation.y += 0.01;
