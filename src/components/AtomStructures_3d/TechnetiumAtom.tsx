...
const TechnetiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Technetium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const technetiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(technetiumNucleus);

    // Create electrons for technetium (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 13 in the fourth orbit, 2 in the fifth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the technetium nucleus
      technetiumNucleus.rotation.x += 0.01;
      technetiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 41; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 13));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 13));
      }

      for (let i = 41; i < 43; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 41) * (2 * Math.PI / 2));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 41) * (2 * Math.PI / 2));
      }
