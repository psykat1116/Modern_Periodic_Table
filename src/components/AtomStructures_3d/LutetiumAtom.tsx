...
const LutetiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const lutetiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(lutetiumNucleus);

    // Create electrons for lutetium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Lutetium
    const electronCounts = [2, 8, 18, 32, 9, 2];
    ...
    const animate = function () {
      ...
      // Rotate the lutetium nucleus
      lutetiumNucleus.rotation.x += 0.01;
      lutetiumNucleus.rotation.y += 0.01;
