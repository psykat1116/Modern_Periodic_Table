...
const ArsenicAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const arsenicNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(arsenicNucleus);

    // Create electrons for arsenic (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 5 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the arsenic nucleus
      arsenicNucleus.rotation.x += 0.01;
      arsenicNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 33; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 5));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 5));
      }
