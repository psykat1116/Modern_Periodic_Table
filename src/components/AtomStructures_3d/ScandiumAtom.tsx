...
const ScandiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const scandiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(scandiumNucleus);

    // Create electrons for scandium (2 in the first orbit, 8 in the second orbit, 9 in the third orbit, 2 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the scandium nucleus
      scandiumNucleus.rotation.x += 0.01;
      scandiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 10; i < 19; i++) {
        electrons[i].position.x = orbitRadius3 * Math.cos(angle3 + (i - 10) * (Math.PI / 8));
        electrons[i].position.y = orbitRadius3 * Math.sin(angle3 + (i - 10) * (Math.PI / 8));
      }

      electrons[19].position.x = orbitRadius4 * Math.cos(angle4);
      electrons[19].position.y = orbitRadius4 * Math.sin(angle4);
