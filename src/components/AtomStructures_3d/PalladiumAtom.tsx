...
const PalladiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Palladium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const palladiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(palladiumNucleus);

    // Create electrons for palladium (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 18 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the palladium nucleus
      palladiumNucleus.rotation.x += 0.01;
      palladiumNucleus.rotation.y += 0.01;
