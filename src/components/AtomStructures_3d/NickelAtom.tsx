...
const NickelAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const nickelNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(nickelNucleus);

    // Create electrons for nickel (2 in the first orbit, 8 in the second orbit, 16 in the third orbit, 2 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the nickel nucleus
      nickelNucleus.rotation.x += 0.01;
      nickelNucleus.rotation.y += 0.01;
      ...
      for (let i = 10; i < 26; i++) {
        electrons[i].position.x = orbitRadius3 * Math.cos(angle3 + (i - 10) * (2 * Math.PI / 16));
        electrons[i].position.y = orbitRadius3 * Math.sin(angle3 + (i - 10) * (2 * Math.PI / 16));
      }

      for (let i = 26; i < 28; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 26) * (2 * Math.PI / 2));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 26) * (2 * Math.PI / 2));
      }
