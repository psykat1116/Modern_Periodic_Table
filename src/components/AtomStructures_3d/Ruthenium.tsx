...
const RutheniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Ruthenium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const rutheniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(rutheniumNucleus);

    // Create electrons for ruthenium (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 15 in the fourth orbit, 1 in the fifth orbit)
    ...
    electrons.push(createElectron(electronMaterial, 10, 1)); // 1 electron in the fifth orbit
    ...
    const orbitRadius5 = 15; // New orbit radius
    ...
    const orbitGeometry5 = new THREE.CircleGeometry(orbitRadius5, orbitSegments); // New orbit geometry
    ...
    const electronOrbit5 = new THREE.LineSegments(new THREE.EdgesGeometry(orbitGeometry5), orbitMaterial); // New orbit

    scene.add(electronOrbit1, electronOrbit2, electronOrbit3, electronOrbit4, electronOrbit5); // Add the new orbit
    ...
    let angle3 = 0;
    let angle4 = 0;
    let angle5 = 0.5; // Initial angle for the fifth orbit

    const animate = function () {
      requestAnimationFrame(animate);

      // Rotate the ruthenium nucleus
      rutheniumNucleus.rotation.x += 0.01;
      rutheniumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 43; i++) { // 43 is the new total number of electrons
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 15));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 15));
      }

      electrons[43].position.x = orbitRadius5 * Math.cos(angle5);
      electrons[43].position.y = orbitRadius5 * Math.sin(angle5);
