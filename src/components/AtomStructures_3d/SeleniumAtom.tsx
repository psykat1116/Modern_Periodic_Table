...
const SeleniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const seleniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(seleniumNucleus);

    // Create electrons for selenium (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 6 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the selenium nucleus
      seleniumNucleus.rotation.x += 0.01;
      seleniumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 34; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 6));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 6));
      }
