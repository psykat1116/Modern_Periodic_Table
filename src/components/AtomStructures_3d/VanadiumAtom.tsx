...
const VanadiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const vanadiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(vanadiumNucleus);

    // Create electrons for vanadium (2 in the first orbit, 8 in the second orbit, 11 in the third orbit, 2 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the vanadium nucleus
      vanadiumNucleus.rotation.x += 0.01;
      vanadiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 21; i < 23; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 21) * (Math.PI));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 21) * (Math.PI));
      }
