...
const PotassiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const potassiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(potassiumNucleus);

    // Create electrons for potassium (2 in the first orbit, 8 in the second orbit, 8 in the third orbit, 1 in the fourth orbit)
    ...
    electrons.push(createElectron(electronMaterial, 8, 1)); // 1 electron in the fourth orbit
    ...
    const animate = function () {
      ...
      // Rotate the potassium nucleus
      potassiumNucleus.rotation.x += 0.01;
      potassiumNucleus.rotation.y += 0.01;
      ...
      electrons[18].position.x = orbitRadius4 * Math.cos(angle4);
      electrons[18].position.y = orbitRadius4 * Math.sin(angle4);
