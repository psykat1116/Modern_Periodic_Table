...
const DarmstadtiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Darmstadtium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const darmstadtiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(darmstadtiumNucleus);

    // Create electrons for darmstadtium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Darmstadtium
    const electronCounts = [2, 8, 18, 32, 32, 16, 2];
    const orbitRadii = [3, 6, 9, 12, 15, 18, 21]; // Adjusted orbit radii
    ...
    const animate = function () {
      ...
      // Rotate the darmstadtium nucleus
      darmstadtiumNucleus.rotation.x += 0.01;
      darmstadtiumNucleus.rotation.y += 0.01;
...
export default DarmstadtiumAtom;