...
const PraseodymiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Praseodymium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const praseodymiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(praseodymiumNucleus);

    // Create electrons for Praseodymium
    ...
    const animate = function () {
      ...
      // Rotate the praseodymium nucleus
      praseodymiumNucleus.rotation.x += 0.01;
      praseodymiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 49; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 21));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 21));
      }

      for (let i = 49; i < 57; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 49) * (2 * Math.PI / 8));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 49) * (2 * Math.PI / 8));
      }

      for (let i = 57; i < 59; i++) {
        electrons[i].position.x = orbitRadius6 * Math.cos(angle6 + (i - 57) * (2 * Math.PI / 2));
        electrons[i].position.y = orbitRadius6 * Math.sin(angle6 + (i - 57) * (2 * Math.PI / 2));
      }
...
export default PraseodymiumAtom;