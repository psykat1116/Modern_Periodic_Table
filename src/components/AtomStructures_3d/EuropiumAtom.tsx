...
const EuropiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const europiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    ...
    // Animation
    let angles = [0, 0, 0, 0, 0, 0.5]; // Initial angles for each orbit

    const animate = function () {
      requestAnimationFrame(animate);

      // Rotate the europium nucleus
      europiumNucleus.rotation.x += 0.01;
      europiumNucleus.rotation.y += 0.01;

      // Move the electrons in orbits
      for (let i = 0; i < electrons.length; i++) {
        const orbitIndex = Math.floor(i / 8); // Determine the current orbit index
        const orbitRadius = [orbitRadius1, orbitRadius2, orbitRadius3, orbitRadius4, orbitRadius5, orbitRadius6][orbitIndex];

        electrons[i].position.x = orbitRadius * Math.cos(angles[orbitIndex] + (i % 8) * (2 * Math.PI / 8));
        electrons[i].position.y = orbitRadius * Math.sin(angles[orbitIndex] + (i % 8) * (2 * Math.PI / 8));
      }

      angles = angles.map((angle) => angle + 0.02); // Increment angles for each orbit
