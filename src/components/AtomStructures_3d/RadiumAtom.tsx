...
const RadiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const radiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(radiumNucleus);

    // Create electrons for radium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Radium
    const electronCounts = [2, 8, 18, 32, 18, 8, 2];
    ...
    const animate = function () {
      ...
      // Rotate the radium nucleus
      radiumNucleus.rotation.x += 0.01;
      radiumNucleus.rotation.y += 0.01;
