...
const HafniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const hafniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(hafniumNucleus);

    // Create electrons for hafnium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Hafnium
    const electronCounts = [2, 8, 18, 32, 10, 2]; // Updated electron counts for Hafnium
    ...
    const animate = function () {
      ...
      // Rotate the hafnium nucleus
      hafniumNucleus.rotation.x += 0.01;
      hafniumNucleus.rotation.y += 0.01;
