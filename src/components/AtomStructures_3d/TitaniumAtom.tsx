...
const TitaniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const titaniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(titaniumNucleus);

    // Create electrons for titanium (2 in the first orbit, 8 in the second orbit, 10 in the third orbit, 2 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the titanium nucleus
      titaniumNucleus.rotation.x += 0.01;
      titaniumNucleus.rotation.y += 0.01;
      ...
      for (let i = 10; i < 20; i++) {
        electrons[i].position.x = orbitRadius3 * Math.cos(angle3 + (i - 10) * (Math.PI / 5));
        electrons[i].position.y = orbitRadius3 * Math.sin(angle3 + (i - 10) * (Math.PI / 5));
      }

      for (let i = 20; i < 22; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 20) * (Math.PI));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 20) * (Math.PI));
      }
