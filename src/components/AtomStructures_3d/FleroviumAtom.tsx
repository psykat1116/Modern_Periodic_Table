...
const FleroviumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Flerovium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const fleroviumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(fleroviumNucleus);

    // Create electrons for flerovium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Flerovium
    const electronCounts = [2, 8, 18, 32, 32, 18, 4];
    ...
    const animate = function () {
      ...
      // Rotate the flerovium nucleus
      fleroviumNucleus.rotation.x += 0.01;
      fleroviumNucleus.rotation.y += 0.01;
