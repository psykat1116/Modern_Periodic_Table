...
const CadmiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const cadmiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(cadmiumNucleus);

    // Create electrons for cadmium (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 18 in the fourth orbit, 2 in the fifth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the cadmium nucleus
      cadmiumNucleus.rotation.x += 0.01;
      cadmiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 46; i < 48; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 46) * (2 * Math.PI / 2));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 46) * (2 * Math.PI / 2));
      }
