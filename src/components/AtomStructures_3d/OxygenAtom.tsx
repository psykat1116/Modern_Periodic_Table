...
const OxygenAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const oxygenNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(oxygenNucleus);

    // Add the symbol "O" to the nucleus
    ...
    fontLoader.load("fonts/helvetiker_bold.typeface.json", (font) => {
      ...
      const textMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0xffffff : 0x000000 });
      const textMesh = new THREE.Mesh(textGeometry, textMaterial);
      textMesh.position.set(-0.15, -0.15, -1); // Adjust the position of the text
      oxygenNucleus.add(textMesh);
    });
    ...
    const animate = function () {
      ...
      // Rotate the oxygen nucleus
      oxygenNucleus.rotation.x += 0.01;
      oxygenNucleus.rotation.y += 0.01;
      ...
      for (let i = 2; i < 8; i++) {
        electrons[i].position.x = orbitRadius2 * Math.cos(angle2 + (i - 2) * (2 * Math.PI) / 6);
        electrons[i].position.y = orbitRadius2 * Math.sin(angle2 + (i - 2) * (2 * Math.PI) / 6);
      }
