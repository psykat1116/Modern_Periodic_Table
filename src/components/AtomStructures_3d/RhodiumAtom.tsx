...
const RhodiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const rhodiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(rhodiumNucleus);

    // Create electrons for rhodium (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 16 in the fourth orbit, 1 in the fifth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the rhodium nucleus
      rhodiumNucleus.rotation.x += 0.01;
      rhodiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 44; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 16));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 16));
      }

      electrons[44].position.x = orbitRadius5 * Math.cos(angle5);
      electrons[44].position.y = orbitRadius5 * Math.sin(angle5);
