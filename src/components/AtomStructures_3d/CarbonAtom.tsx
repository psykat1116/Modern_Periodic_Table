...
const CarbonAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const carbonNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(carbonNucleus);

    // Create electrons for carbon (2 electrons in the first orbit, 4 electrons in the second orbit)
    ...
    const animate = function () {
      ...
      // Rotate the carbon nucleus
      carbonNucleus.rotation.x += 0.01;
      carbonNucleus.rotation.y += 0.01;
      ...
      for (let i = 0; i < 4; i++) {
        electrons[i + 2].position.x = orbitRadius2 * Math.cos(angle2 + (i * (2 * Math.PI) / 4));
        electrons[i + 2].position.y = orbitRadius2 * Math.sin(angle2 + (i * (2 * Math.PI) / 4));
      }
