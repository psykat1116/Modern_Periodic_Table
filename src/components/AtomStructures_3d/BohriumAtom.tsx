...
const BohriumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const bohriumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(bohriumNucleus);

    // Create electrons for Bohrium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Bohrium
    const electronCounts = [2, 8, 18, 32, 32, 13, 2]; // Bohrium electron counts
    const orbitRadii = [3, 6, 9, 12, 15, 18, 21]; // Bohrium orbit radii
    ...
    const animate = function () {
      ...
      // Rotate the bohrium nucleus
      bohriumNucleus.rotation.x += 0.01;
      bohriumNucleus.rotation.y += 0.01;
