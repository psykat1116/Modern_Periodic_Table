...
const YtterbiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Ytterbium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const ytterbiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(ytterbiumNucleus);

    // Create electrons for ytterbium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Ytterbium
    const electronCounts = [2, 8, 18, 32, 8, 2];
    ...
    const animate = function () {
      ...
      // Rotate the ytterbium nucleus
      ytterbiumNucleus.rotation.x += 0.01;
      ytterbiumNucleus.rotation.y += 0.01;
