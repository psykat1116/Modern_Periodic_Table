...
const NeptuniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Neptunium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const neptuniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(neptuniumNucleus);

    // Create electrons for neptunium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x00ff00 : 0x00ff00 });

    // Electron distribution for Neptunium
    const electronCounts = [2, 8, 18, 32, 22, 9, 2]; // Updated electron distribution
    const orbitRadii = [3, 6, 9, 12, 15, 18, 21]; // Updated orbit radii
    ...
    const animate = function () {
      ...
      // Rotate the neptunium nucleus
      neptuniumNucleus.rotation.x += 0.01;
      neptuniumNucleus.rotation.y += 0.01;
