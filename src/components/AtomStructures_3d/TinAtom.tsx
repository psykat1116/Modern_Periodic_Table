...
const TinAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const tinNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(tinNucleus);

    // Create electrons for tin (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 18 in the fourth orbit, 4 in the fifth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the tin nucleus
      tinNucleus.rotation.x += 0.01;
      tinNucleus.rotation.y += 0.01;
      ...
      for (let i = 46; i < 50; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 46) * (2 * Math.PI / 4));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 46) * (2 * Math.PI / 4));
      }
