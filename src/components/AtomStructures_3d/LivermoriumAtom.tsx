...
const LivermoriumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Livermorium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const livermoriumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(livermoriumNucleus);

    // Create electrons for Livermorium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Livermorium
    const electronCounts = [2, 8, 18, 32, 32, 18, 6];
    const orbitRadii = [3, 6, 9, 12, 15, 18, 21];
    ...
    // Position camera
    camera.position.z = 30; // Adjusted for the additional orbit
    ...
    const animate = function () {
      ...
      // Rotate the livermorium nucleus
      livermoriumNucleus.rotation.x += 0.01;
      livermoriumNucleus.rotation.y += 0.01;
...
export default LivermoriumAtom;