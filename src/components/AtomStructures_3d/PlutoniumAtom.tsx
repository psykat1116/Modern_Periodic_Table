...
const PlutoniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Plutonium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const plutoniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(plutoniumNucleus);

    // Create electrons for plutonium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Plutonium
    const electronCounts = [2, 8, 18, 32, 24, 8, 2]; // Updated electron counts
    ...
    const animate = function () {
      ...
      // Rotate the plutonium nucleus
      plutoniumNucleus.rotation.x += 0.01;
      plutoniumNucleus.rotation.y += 0.01;
