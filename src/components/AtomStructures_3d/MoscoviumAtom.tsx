...
const MoscoviumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Moscovium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const moscoviumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(moscoviumNucleus);

    // Create electrons for Moscovium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Moscovium
    const electronCounts = [2, 8, 18, 32, 32, 18, 5];
    ...
    const animate = function () {
      ...
      // Rotate the moscovium nucleus
      moscoviumNucleus.rotation.x += 0.01;
      moscoviumNucleus.rotation.y += 0.01;
