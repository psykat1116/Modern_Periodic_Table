...
const LeadAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const leadNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(leadNucleus);

    // Create electrons for lead
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Lead
    const electronCounts = [2, 8, 18, 32, 18, 4];
    ...
    const animate = function () {
      ...
      // Rotate the lead nucleus
      leadNucleus.rotation.x += 0.01;
      leadNucleus.rotation.y += 0.01;
