...
const SeaborgiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Seaborgium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const seaborgiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(seaborgiumNucleus);

    // Create electrons for seaborgium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Seaborgium
    const electronCounts = [2, 8, 18, 32, 32, 12, 2];
    ...
    const animate = function () {
      ...
      // Rotate the seaborgium nucleus
      seaborgiumNucleus.rotation.x += 0.01;
      seaborgiumNucleus.rotation.y += 0.01;
