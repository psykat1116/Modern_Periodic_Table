...
const PromethiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Promethium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const promethiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(promethiumNucleus);

    // Create electrons for promethium
    ...
    const orbitRadius6 = 18; // Adding a new orbit for Promethium
    ...
    const orbitGeometry6 = new THREE.CircleGeometry(orbitRadius6, orbitSegments); // New orbit for Promethium
    ...
    const electronOrbit6 = new THREE.LineSegments(new THREE.EdgesGeometry(orbitGeometry6), orbitMaterial); // New orbit for Promethium

    scene.add(electronOrbit1, electronOrbit2, electronOrbit3, electronOrbit4, electronOrbit5, electronOrbit6); // Add new orbit to the scene

    // Position camera
    camera.position.z = 21; // Adjusted for the additional orbit
    ...
    let angle4 = 0;
    let angle5 = 0;
    let angle6 = 0.5; // Initial angle for the sixth orbit

    const animate = function () {
      requestAnimationFrame(animate);

      // Rotate the promethium nucleus
      promethiumNucleus.rotation.x += 0.01;
      promethiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 51; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 23));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 23));
      }

      for (let i = 51; i < 59; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 51) * (2 * Math.PI / 8));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 51) * (2 * Math.PI / 8));
      }

      for (let i = 59; i < 61; i++) {
        electrons[i].position.x = orbitRadius6 * Math.cos(angle6 + (i - 59) * (2 * Math.PI / 2));
        electrons[i].position.y = orbitRadius6 * Math.sin(angle6 + (i - 59) * (2 * Math.PI / 2));
      }
