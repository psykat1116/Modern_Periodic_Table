...
const GermaniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Germanium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const germaniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(germaniumNucleus);

    // Create electrons for germanium (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 4 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the germanium nucleus
      germaniumNucleus.rotation.x += 0.01;
      germaniumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 32; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 4));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 4));
      }
