...
const ErbiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const erbiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(erbiumNucleus);

    // Create electrons for erbium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Erbium
    const electronCounts = [2, 8, 18, 30, 8, 2];
    ...
    const animate = function () {
      ...
      // Rotate the erbium nucleus
      erbiumNucleus.rotation.x += 0.01;
      erbiumNucleus.rotation.y += 0.01;
