...
const AstatineAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const astatineNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(astatineNucleus);

    // Create electrons for astatine
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Astatine
    const electronCounts = [2, 8, 18, 32, 18, 7];
    ...
    const animate = function () {
      ...
      // Rotate the astatine nucleus
      astatineNucleus.rotation.x += 0.01;
      astatineNucleus.rotation.y += 0.01;
