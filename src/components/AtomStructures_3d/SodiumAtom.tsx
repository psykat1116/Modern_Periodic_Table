...
const SodiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const sodiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(sodiumNucleus);

    // Create electrons for sodium (2 in the first orbit, 8 in the second orbit, 1 in the third orbit)
    ...
    electrons.push(createElectron(electronMaterial, 6, 1)); // 1 electron in the third orbit
    ...
    const animate = function () {
      ...
      // Rotate the sodium nucleus
      sodiumNucleus.rotation.x += 0.01;
      sodiumNucleus.rotation.y += 0.01;
      ...
      electrons[10].position.x = orbitRadius3 * Math.cos(angle3);
      electrons[10].position.y = orbitRadius3 * Math.sin(angle3);
