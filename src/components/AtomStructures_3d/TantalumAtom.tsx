...
const TantalumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const tantalumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(tantalumNucleus);

    // Create electrons for tantalum
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Tantalum
    const electronCounts = [2, 8, 18, 32, 11, 2];
    ...
    const animate = function () {
      ...
      // Rotate the tantalum nucleus
      tantalumNucleus.rotation.x += 0.01;
      tantalumNucleus.rotation.y += 0.01;
