...
const NitrogenAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const nitrogenNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(nitrogenNucleus);

    // Create electrons for nitrogen (2 electrons in the first orbit, 5 electrons in the second orbit)
    ...
    const animate = function () {
      ...
      // Rotate the nitrogen nucleus
      nitrogenNucleus.rotation.x += 0.01;
      nitrogenNucleus.rotation.y += 0.01;
      ...
      for (let i = 2; i < electrons.length; i++) {
        const orbitRadius = i < 7 ? orbitRadius2 : orbitRadius1; // Use larger orbit for last 5 electrons
        electrons[i].position.x = orbitRadius * Math.cos(angle2 + (i - 2) * (Math.PI / 2.5));
        electrons[i].position.y = orbitRadius * Math.sin(angle2 + (i - 2) * (Math.PI / 2.5));
      }
