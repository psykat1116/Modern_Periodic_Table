...
const SilverAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: 0xC0C0C0 }); // Silver color
    const silverNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    ...
    const orbitMaterial = new THREE.LineBasicMaterial({ color: 0xC0C0C0 }); // Silver color
    ...
    const animate = function () {
      ...
      // Rotate the silver nucleus
      silverNucleus.rotation.x += 0.01;
      silverNucleus.rotation.y += 0.01;
      ...
      electrons[46].position.x = orbitRadius5 * Math.cos(angle5);
      electrons[46].position.y = orbitRadius5 * Math.sin(angle5);
