...
const ProtactiniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Protactinium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const protactiniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(protactiniumNucleus);

    // Create electrons for protactinium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Protactinium
    const electronCounts = [2, 8, 18, 32, 20, 9, 2];
    const orbitRadii = [3, 6, 9, 12, 15, 18, 21]; // Adjusted radii for Protactinium
    ...
    const animate = function () {
      ...
      // Rotate the protactinium nucleus
      protactiniumNucleus.rotation.x += 0.01;
      protactiniumNucleus.rotation.y += 0.01;
...
export default ProtactiniumAtom;