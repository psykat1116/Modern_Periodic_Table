...
const OganessonAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Oganesson atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const oganessonNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(oganessonNucleus);

    // Create electrons for Oganesson
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Oganesson
    const electronCounts = [2, 8, 18, 32, 32, 18, 8];
    ...
    const animate = function () {
      ...
      // Rotate the oganesson nucleus
      oganessonNucleus.rotation.x += 0.01;
      oganessonNucleus.rotation.y += 0.01;
