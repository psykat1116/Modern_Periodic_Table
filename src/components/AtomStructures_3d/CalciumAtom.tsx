...
const CalciumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const calciumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(calciumNucleus);

    // Create electrons for calcium (2 in the first orbit, 8 in the second orbit, 8 in the third orbit, 2 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the calcium nucleus
      calciumNucleus.rotation.x += 0.01;
      calciumNucleus.rotation.y += 0.01;
      ...
      for (let i = 10; i < 18; i++) {
        electrons[i].position.x = orbitRadius3 * Math.cos(angle3 + (i - 10) * (Math.PI / 4));
        electrons[i].position.y = orbitRadius3 * Math.sin(angle3 + (i - 10) * (Math.PI / 4));
      }

      for (let i = 18; i < 20; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 18) * (Math.PI));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 18) * (Math.PI));
      }
