...
const BoronAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Boron atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32); // Adjusted size for Boron nucleus
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const boronNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(boronNucleus);

    // Create electrons for boron (2 electrons in the first orbit, 3 electrons in the second orbit)
    ...
    const animate = function () {
      ...
      // Rotate the boron nucleus
      boronNucleus.rotation.x += 0.01;
      boronNucleus.rotation.y += 0.01;
      ...
      for (let i = 2; i < 5; i++) {
        electrons[i].position.x = orbitRadius2 * Math.cos(angle2 + ((i - 2) * (2 * Math.PI) / 3));
        electrons[i].position.y = orbitRadius2 * Math.sin(angle2 + ((i - 2) * (2 * Math.PI) / 3));
      }
