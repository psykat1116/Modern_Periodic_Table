...
const MolybdenumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Molybdenum atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const molybdenumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(molybdenumNucleus);

    // Create electrons for Molybdenum
    ...
    const orbitRadius5 = 15; // Additional orbit for Molybdenum
    ...
    const animate = function () {
      ...
      // Rotate the molybdenum nucleus
      molybdenumNucleus.rotation.x += 0.01;
      molybdenumNucleus.rotation.y += 0.01;
      ...
      electrons[41].position.x = orbitRadius5 * Math.cos(angle5);
      electrons[41].position.y = orbitRadius5 * Math.sin(angle5);
