...
const FerrousAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const ironNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(ironNucleus);

    // Create electrons for iron (2 in the first orbit, 8 in the second orbit, 14 in the third orbit, 2 in the fourth orbit)
    ...
    electrons.push(createElectron(electronMaterial, 8, 1)); // 2 electrons in the fourth orbit

    electrons.push(createElectron(electronMaterial, 8, 2));
    ...
    const animate = function () {
      ...
      // Rotate the iron nucleus
      ironNucleus.rotation.x += 0.01;
      ironNucleus.rotation.y += 0.01;
      ...
      for (let i = 2; i < 10; i++) {
        electrons[i].position.x = orbitRadius2 * Math.cos(angle2 + (i - 2) * (Math.PI / 7));
        electrons[i].position.y = orbitRadius2 * Math.sin(angle2 + (i - 2) * (Math.PI / 7));
      }

      for (let i = 10; i < 24; i++) {
        electrons[i].position.x = orbitRadius3 * Math.cos(angle3 + (i - 10) * (Math.PI / 14));
        electrons[i].position.y = orbitRadius3 * Math.sin(angle3 + (i - 10) * (Math.PI / 14));
      }

      electrons[24].position.x = orbitRadius4 * Math.cos(angle4);
      electrons[24].position.y = orbitRadius4 * Math.sin(angle4);

      electrons[25].position.x = orbitRadius4 * Math.cos(angle4 + Math.PI);
      electrons[25].position.y = orbitRadius4 * Math.sin(angle4 + Math.PI);
