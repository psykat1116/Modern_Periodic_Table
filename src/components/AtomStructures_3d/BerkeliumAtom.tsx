...
const BerkeliumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Berkelium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const berkeliumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(berkeliumNucleus);

    // Create electrons for berkelium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Berkelium
    const electronCounts = [2, 8, 18, 32, 27, 8, 2];
    ...
    const animate = function () {
      ...
      // Rotate the berkelium nucleus
      berkeliumNucleus.rotation.x += 0.01;
      berkeliumNucleus.rotation.y += 0.01;
