...
const HassiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const hassiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(hassiumNucleus);

    // Create electrons for Hassium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Hassium
    const electronCounts = [2, 8, 18, 32, 32, 14, 2];
    const orbitRadii = [3, 6, 9, 12, 15, 18, 21]; // Adjusted radii for Hassium
    ...
    const animate = function () {
      ...
      // Rotate the hassium nucleus
      hassiumNucleus.rotation.x += 0.01;
      hassiumNucleus.rotation.y += 0.01;
