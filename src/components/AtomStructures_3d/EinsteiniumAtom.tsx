...
const EinsteiniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Einsteinium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const einsteiniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(einsteiniumNucleus);

    // Create electrons for einsteinium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Einsteinium
    const electronCounts = [2, 8, 18, 32, 29, 8, 2]; // Updated electron counts for Einsteinium
    ...
    const animate = function () {
      ...
      // Rotate the einsteinium nucleus
      einsteiniumNucleus.rotation.x += 0.01;
      einsteiniumNucleus.rotation.y += 0.01;
...
export default EinsteiniumAtom;