...
const RubidiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const rubidiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(rubidiumNucleus);

    // Create electrons for rubidium (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 8 in the fourth orbit, 1 in the fifth orbit)
    ...
    electrons.push(createElectron(electronMaterial, 10, 1));
    ...
    const animate = function () {
      ...
      // Rotate the rubidium nucleus
      rubidiumNucleus.rotation.x += 0.01;
      rubidiumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 36; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 8));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 8));
      }

      electrons[36].position.x = orbitRadius5 * Math.cos(angle5);
      electrons[36].position.y = orbitRadius5 * Math.sin(angle5);
