...
const ActiniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const actiniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(actiniumNucleus);

    // Create electrons for Actinium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Actinium
    const electronCounts = [2, 8, 18, 32, 18, 9, 2];
    const orbitRadii = [3, 6, 9, 12, 15, 18, 21]; // Adjusted radii for Actinium
    ...
    const animate = function () {
      ...
      // Rotate the actinium nucleus
      actiniumNucleus.rotation.x += 0.01;
      actiniumNucleus.rotation.y += 0.01;
