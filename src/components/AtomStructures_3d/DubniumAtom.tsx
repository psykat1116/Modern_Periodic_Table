...
const DubniumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const dubniumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(dubniumNucleus);

    // Create electrons for Dubnium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Dubnium
    const electronCounts = [2, 8, 18, 32, 32, 11, 2]; // Updated electron counts
    ...
    const animate = function () {
      ...
      // Rotate the dubnium nucleus
      dubniumNucleus.rotation.x += 0.01;
      dubniumNucleus.rotation.y += 0.01;
