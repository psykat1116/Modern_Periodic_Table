...
const ThuliumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const thuliumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(thuliumNucleus);

    // Create electrons for thulium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Thulium
    const electronCounts = [2, 8, 18, 31, 8, 2]; // Adjusted for Thulium
    ...
    const animate = function () {
      ...
      // Rotate the thulium nucleus
      thuliumNucleus.rotation.x += 0.01;
      thuliumNucleus.rotation.y += 0.01;
