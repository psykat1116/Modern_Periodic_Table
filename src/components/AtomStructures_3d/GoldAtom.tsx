...
const GoldAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffd700 }); // Gold color
    const goldNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(goldNucleus);

    // Create electrons for Gold
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Gold
    const electronCounts = [2, 8, 18, 32, 18, 1]; // Electron configuration of Gold (Au)
    ...
    const animate = function () {
      ...
      // Rotate the gold nucleus
      goldNucleus.rotation.x += 0.01;
      goldNucleus.rotation.y += 0.01;
