...
const AmericiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    // Americium atom model
    const nucleusGeometry = new THREE.SphereGeometry(1, 32, 32);
    const nucleusMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x333333 : 0xffffff });
    const americiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(americiumNucleus);

    // Create electrons for americium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Americium
    const electronCounts = [2, 8, 18, 32, 25, 8, 2];
    ...
    const animate = function () {
      ...
      // Rotate the americium nucleus
      americiumNucleus.rotation.x += 0.01;
      americiumNucleus.rotation.y += 0.01;
