...
const LithiumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const lithiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(lithiumNucleus);

    // Create electrons for lithium (2 electrons in the first orbit, 1 electron in the second orbit)
    ...
    electrons.push(createElectron(electronMaterial, 4, 1)); // 1 electron in the second orbit
    ...
    const animate = function () {
      ...
      // Rotate the lithium nucleus
      lithiumNucleus.rotation.x += 0.01;
      lithiumNucleus.rotation.y += 0.01;
