...
const CuriumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const curiumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(curiumNucleus);

    // Create electrons for curium
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Curium
    const electronCounts = [2, 8, 18, 32, 25, 9, 2]; // Updated electron counts for Curium
    const orbitRadii = [3, 6, 9, 12, 15, 18, 21]; // Updated orbit radii for Curium
    ...
    const animate = function () {
      ...
      // Rotate the curium nucleus
      curiumNucleus.rotation.x += 0.01;
      curiumNucleus.rotation.y += 0.01;
