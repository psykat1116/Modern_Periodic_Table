...
const GalliumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const galliumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(galliumNucleus);

    // Create electrons for gallium (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 3 in the fourth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the gallium nucleus
      galliumNucleus.rotation.x += 0.01;
      galliumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 31; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 3));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 3));
      }
