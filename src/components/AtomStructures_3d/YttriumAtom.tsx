...
const YttriumAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const yttriumNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(yttriumNucleus);

    // Create electrons for yttrium (2 in the first orbit, 8 in the second orbit, 18 in the third orbit, 9 in the fourth orbit, 2 in the fifth orbit)
    ...
    const animate = function () {
      ...
      // Rotate the yttrium nucleus
      yttriumNucleus.rotation.x += 0.01;
      yttriumNucleus.rotation.y += 0.01;
      ...
      for (let i = 28; i < 37; i++) {
        electrons[i].position.x = orbitRadius4 * Math.cos(angle4 + (i - 28) * (2 * Math.PI / 9));
        electrons[i].position.y = orbitRadius4 * Math.sin(angle4 + (i - 28) * (2 * Math.PI / 9));
      }

      for (let i = 37; i < 39; i++) {
        electrons[i].position.x = orbitRadius5 * Math.cos(angle5 + (i - 37) * (2 * Math.PI / 2));
        electrons[i].position.y = orbitRadius5 * Math.sin(angle5 + (i - 37) * (2 * Math.PI / 2));
      }
