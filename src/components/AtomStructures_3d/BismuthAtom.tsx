...
const BismuthAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const bismuthNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(bismuthNucleus);

    // Create electrons for Bismuth
    const electrons: THREE.Mesh[] = [];
    const electronMaterial = new THREE.MeshBasicMaterial({ color: theme === "dark" ? 0x0000ff : 0xff0000 });

    // Electron distribution for Bismuth
    const electronCounts = [2, 8, 18, 32, 18, 5];
    ...
    const animate = function () {
      ...
      // Rotate the bismuth nucleus
      bismuthNucleus.rotation.x += 0.01;
      bismuthNucleus.rotation.y += 0.01;
