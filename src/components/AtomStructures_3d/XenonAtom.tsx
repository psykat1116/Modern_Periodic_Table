...
const XenonAtom = ({ theme, containerSize }: { theme: string, containerSize: { width: number, height: number } }) => {
  ...
  useEffect(() => {
    ...
    const xenonNucleus = new THREE.Mesh(nucleusGeometry, nucleusMaterial);
    scene.add(xenonNucleus);

    // Create electrons for xenon
    ...
    const animate = function () {
      ...
      // Rotate the xenon nucleus
      xenonNucleus.rotation.x += 0.01;
      xenonNucleus.rotation.y += 0.01;
